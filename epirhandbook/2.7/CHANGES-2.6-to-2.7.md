...
### C4. `gis` — currently excluded either way
The `gis` chapter was already excluded from the book before this upgrade (it fetches live
OpenStreetMap tiles at render time). It remains unrenderable. No action needed unless you want
it back, which would be a rewrite.
